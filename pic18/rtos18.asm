...
;    あり (PIC18 で 31 段)。PIC18 は TOS (TOSU:TOSH:TOSL) と STKPTR で
;    てっぺんを読み書きできるが、普通の CPU のように SP 一つで丸ごと
;    付け替えることはできない。そこで文脈切り替えのたびに、走行中タスクの
;    戻り番地を TOS からタスク制御ブロック(TCB)へ移し、次タスクのを積み直す。
